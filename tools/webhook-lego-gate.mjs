...
gate('W03', 'webhook routing conformance suite', () => { const out = run(['--test', 'test/*.test.mjs'], pkg); const pass = /^# pass (\d+)$/m.exec(out)?.[1]; const fail = /^# fail (\d+)$/m.exec(out)?.[1]; if (pass !== '24' || fail !== '0') throw new Error(`${pass}/${fail}`); return '24 pass / 0 fail'; });
gate('W04', 'reference tree remains pinned', () => run(['tools/workflow-reference-manifest.mjs', '--check']).trim().split('\n').at(-1));
gate('W05', 'formal webhook contract present', () => { const c = readFileSync(join(root, 'contracts/webhook.contract.md'), 'utf8'); for (const s of ['WebhookService', 'WebhookRequestHandler', 'findWebhook', 'deleteWorkflowWebhooks', 'WebhookHttpServer', 'WaitingWebhookManager']) if (!c.includes(s)) throw new Error(`missing ${s}`); return '6/6 core surfaces contracted'; });
