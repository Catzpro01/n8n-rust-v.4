...
function listFiles(ref, scope) {
	const out = git(['ls-tree', '-r', '--name-only', ref]);
	return new Set(
		out
			.split('\n')
			.map((line) => line.trim())
			.filter((line) => line.length > 0 && (!scope || line.startsWith(scope))),
	);
}

function blobHash(ref, path) {
	try {
		return git(['rev-parse', `${ref}:${path}`]).trim();
	} catch {
		return null;
	}
}
...
for (const ref of targets) {
	try {
		trees.set(ref, listFiles(ref, scope));
	} catch (error) {
...
for (let i = 0; i < targets.length; i++) {
	for (let j = i + 1; j < targets.length; j++) {
		...
		const shared = [...filesA].filter((path) => filesB.has(path));
		if (shared.length === 0) continue;

		let differing = 0;
		for (const path of shared) {
			const ha = blobHash(a, path);
			const hb = blobHash(b, path);
			if (ha && hb && ha !== hb) {
				differing += 1;
		...
		console.log(
			`\n${shortLabel(a)}  <->  ${shortLabel(b)}\n` +
				`  shared paths: ${shared.length}   differing content: ${differing}`,
		);
	}
}
