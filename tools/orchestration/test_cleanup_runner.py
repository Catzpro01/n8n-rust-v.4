...
class GitFixture:
    ...
    def __init__(self, commits: int = 6):
        ...
        refspecs = [f"main:{b}" for b in (self.SLICE, "arena-manager")]
...
class TestPermanentBranches(unittest.TestCase):
    def test_permanent_names_are_recognised(self):
        for name in ("main", "master", "arena-manager", "refs/heads/main", " arena-manager ", "refs/heads/arena-manager"):
            self.assertTrue(cr.is_permanent_branch(name), name)

    def test_task_and_slice_branches_stay_deletable(self):
        for name in ("arena/manager/P5-M04", "arena/manager/dec-0017-promote", "arena/agent-01/TASK-0001", "arena/agent-01", "arena/agent-10",
...
class TestMainFlow(unittest.TestCase):
    ...
    def test_never_deletes_permanent_branches_whatever_the_head_was(self):
        ...
        for head in ("main", "arena-manager", "refs/heads/main", "refs/heads/arena-manager"):
...
class TestDeletionWithFakeGit(unittest.TestCase):
    ...
    def test_permanent_branch_never_reaches_git(self):
        git = FakeGit([])
        log: List[str] = []
        self.assertEqual(cr.delete_head_branch(git, "origin", "arena-manager", log.append), cr.PROTECTED)
