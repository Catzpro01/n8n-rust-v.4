...
8. `CycleDetection` considers only `main` connections; `ai_*` edges are excluded. The back-edge `path` is `['connections', <from>, 'main']` (no output index); only the message text and the `path[1]` node name are stable guarantees — do not pin the array length.
9. **Determinism / parity fixtures:** error order is defined independent of JSON key order — nodes in `nodes[]` order; connection sources in `nodes[]` order first, then unknown sources sorted lexically; connection types sorted lexically; outputs/targets in array order. Any port must reproduce `tests/reference/agent-4/validation/fixtures/*.json` (`expected` after key-sorted canonicalisation) byte-for-byte; regenerate with `gen-fixtures.ts` only when the contract changes.
10. Any implementation of the new rules must land in files inside this boundary and be documented as NEW CAPABILITY in `docs/isolation/validation.md`; no other LEGO may implement uniqueness/dangling/cycle checks (ISSUE-003 Option A).
