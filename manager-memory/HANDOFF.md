...
| MAIN | `76e4f2a5` (merge of #333, P7 authorization DEC-0024). Verify it against the remote; a parallel session also commits to main |
...
| NEXT_ACTION | **OWNER (2026-09-26): P7 marathon, never waiting on self-hosted runners.** DEC-0024 (OWNER, ACTIVE, not yet canonical) split P7 into P7-S01..S08 = #223 §42 rungs P7.1..P7.8 (#333 -> `76e4f2a5`). P7-S01 (Parameter Contract & Compiler, `src/lego/parameter-plan.mjs`) is delivery PR #334 (head `df84c218`). Per slice: one delivery PR -> hosted green, self-hosted green or WAITING_RUNNER -> merge -> fresh-main verification -> one governance PR (register evidence/pointer). A slice becomes implemented only when DEC-0015 holds; otherwise it stays in-progress (VERIFYING). Next: P7-S02 visibility + dependency graph (port of upstream displayParameter/checkConditions; expression short-circuit in show is key-order dependent) |
