...
| MAIN | `ad30edcf` (merge of #335, governance P7-S01 VERIFYING + start P7-S02). Verify it against the remote; a parallel session also commits to main |
...
| NEXT_ACTION | **OWNER (2026-09-26): P7 marathon, never waiting on self-hosted runners.** DEC-0024 split P7 into P7-S01..S08 (= #223 §42 P7.1..P7.8). **P7-S01 MERGED** as #334 -> `8aedbfa2` (head `71510167`) and **VERIFYING** in register `verifyingSlices` (#335 -> `ad30edcf`): 6 self-hosted checks WAITING_RUNNER. When they pass on main, a governance PR flips it to implemented (mergeSha `8aedbfa2`, features P7-F-PARAM-001/002/021). **P7-S02** (in-progress) is delivery PR #336, head `3d1dcd02` (`src/lego/parameter-graph.mjs`: displayParameter port, dependency graph, incremental ResolutionSession). Next: merge #336 when hosted checks are green -> verify main -> governance PR (P7-S02 into verifyingSlices, start P7-S03) -> P7-S03 validation/normalization (port of getNodeParameters :658 and getNodeParametersIssues :1202). Drafts are backed up in manager-state/p7-s02/. Per slice: one delivery PR -> hosted green, self-hosted green or WAITING_RUNNER -> merge -> fresh-main verification -> one governance PR. |
