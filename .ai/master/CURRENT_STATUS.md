...
| **Realtime Delivery Progress** | **89.6%** (P0–P11 checkpoint-weighted; future programs excluded; no checkpoint model contributes 0) |
...
- **P4-S01** (P4) — planned · realtime 78.0% · completion contribution 0.0% · current checkpoint — · latest completed CP-03 (completed) · updated 2026-09-25T21:31:26Z — CP-03 complete (20%, cumulative 78%). P4 self-profiling delivered. It is optional and off by default so the cost is zero when unused, and it records all six required signals — route lookup time, admission time, queue wait, payload-copy cost, allocation pressure and slow route patterns. Collection is hard-bounded by max_samples with an overflow counter instead of unbounded memory (8 recorded / 24 dropped of 32 in the bounding test), derived state is resettable, and eBPF is proven complementary only: the internal profile keeps recording when the EbpfComplement backend is selected, so eBPF never becomes a P4 runtime dependency.
