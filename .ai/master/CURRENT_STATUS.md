...
| **Realtime Delivery Progress** | **89.3%** (P0–P11 checkpoint-weighted; future programs excluded; no checkpoint model contributes 0) |
...
- **P4-S01** (P4) — planned · realtime 34.0% · completion contribution 0.0% · current checkpoint — · latest completed CP-01 (completed) · updated 2026-09-25T21:31:10Z — CP-01 complete (34%). Predictive admission controller delivered and the P4.8 adapt_limits no-op repaired. The controller observes all eight required signals — latency, queue depth, CPU pressure, memory pressure, admission rate, completion rate, error rate and recent burst shape — and adjusts only the five declared knobs. Hard ceilings are proven immutable across 64 randomised pressure steps; queue budget is proven bounded by the observed queue capacity so an unbounded queue cannot form; AdmissionTargets is proven to carry no security/permission/authentication field; and stale telemetry is proven to fall back conservatively rather than aggressively. Hysteresis stops target flapping while still letting a genuine jump through, and the first observation is no longer swallowed by the deadband.
