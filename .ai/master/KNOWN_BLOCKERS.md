...
| B-9 | Legacy i18n ownership is unresolved | `domains.json → legacy-rest.unresolvedOwnership`: `/rest/credential-translation`, `/rest/node-translation-headers` are constant empty objects with no business logic | translation must not be forced into `credentials`, `node-registry` or `workflow`; the decision belongs to the manager (XA-14) | manager + agent-2 |
