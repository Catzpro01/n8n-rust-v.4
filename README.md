...
Checkpoint-weighted earned points / current-delivery points: 13078 / 14600. Denominator: programs P0–P11 only (146 active slices). Future programs are excluded. 4 slice(s) declare checkpoints. 127 implemented slice(s) have no checkpoint list and contribute 100 each. 15 slice(s) have no checkpoint model and contribute 0. A 0 from a missing model is not a measured fraction of that slice.
...
P4   realtime ████████████████████  97.8%  completion  90.0%  9/10  status complete
...
| P4 | Trigger / Webhook / Ingress | 97.8% | 90.0% | 9/10 | complete |
...
```text
 1. `P4-S01` planned; realtime 78.0%; completion contribution 0.0%; updated 2026-09-25T21:31:26Z
...
- **Realtime Delivery Progress:** **97.8%** `████████████████████ 97.8%`
...
| `P4-S01` | Ingress innovation remainder | predictive admission controller (full), shadow compatibility path, P4 self-profiling, dedicated webhook/ingress plane | 🟡 Planned | 78.0% | 0.0% | — | CP-03 P4 self-profiling: optional, low-overhead, hard-bounded, with eBPF only complementary (completed, 20) | — |
...
- **Freshness:** generated by `npm run lego:ai` from register 2.3.0 (fingerprint `f4aa6aeb6c877bfc`); `npm run lego:ai:check` fails when this section, the `.ai` pack or the register disagree.
