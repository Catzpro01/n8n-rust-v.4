...
Checkpoint-weighted earned points / current-delivery points: 13000 / 14600. Denominator: programs P0–P11 only (146 active slices). Future programs are excluded. 4 slice(s) declare checkpoints. 127 implemented slice(s) have no checkpoint list and contribute 100 each. 15 slice(s) have no checkpoint model and contribute 0. A 0 from a missing model is not a measured fraction of that slice.
...
```text
 1. `P4-S01` planned; realtime 0.0%; completion contribution 0.0%; updated 2026-09-25T21:30:38Z
...
| `P4-S01` | Ingress innovation remainder | predictive admission controller (full), shadow compatibility path, P4 self-profiling, dedicated webhook/ingress plane | 🟡 Planned | 0.0% | 0.0% | CP-01 Predictive admission controller (full): telemetry surface, bounded prediction, hysteresis, and the adapt_limits clamp repair (in-progress, 34) | — | — |
...
- **Freshness:** generated by `npm run lego:ai` from register 2.3.0 (fingerprint `2f4bf13ae4dd5489`); `npm run lego:ai:check` fails when this section, the `.ai` pack or the register disagree.
