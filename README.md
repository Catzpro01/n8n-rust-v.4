...
P2   realtime ███████████████████░  97.1%  completion  94.1%  32/34  status complete
...
| P2 | LEGO / AI / Plugin Foundation | 97.1% | 94.1% | 32/34 | complete |
...
- **Slice Completion:** **94.1%** (32 / 34 implemented). Remaining 2. 32 implemented, 2 blocked.
- **Program status:** complete. Program status is not a percentage and is not 100% just because the word is complete.
...
- **Checkpoint model:** 1 slice(s) declare checkpoints; 32 implemented slice(s) use the legacy 100 rule; 1 slice(s) have no checkpoint model and stay at 0.0%.
...
| `P2-S02` | Frontend LEGO shared notification surface + accessibility parity | Frontend LEGO shared notification surface + accessibility parity | 🔴 Blocked | 100.0% | 0.0% | — | CP-06 Gate staleness fixed and the single-pilot rule generalized (completed, 10) | Self-hosted runner fleet offline. Delivery PR #332 (head c62846f857e9e4ec1fc7525d6d4c191f7a3dfb23) is green on all 3 GitHub-hosted jobs, but the 6 required self-hosted jobs have never left queued: Level 0 (Check & Format), Level 1 (Affected Tests), Level 2 Conformance LEGO & Node Catalog, Level 2 Workspace Tests (linux), Level 2 Workspace Tests (windows), Windows worker portability probe. DEC-0015 retry attempted on all 6 (jobs 108338293120, 108338293185, 108338293240, 108338293361, 108338293536, 108338293770) — each returned HTTP 403 already-running and stayed queued, so no runner ever picked them up. mergeable_state is unstable, which is not green. The same fleet outage already blocks governance PR #331 (P6-S02). Implementation, evidence and all 6 checkpoints are complete; only the merge is blocked. |
| `P2-S03` | Frontend Evolution layers 3-6 | Frontend Evolution layers 3-6 (core workflow, platform, AI surfaces, legacy UI decommission) | 🔴 Blocked | 0.0% | 0.0% | none declared | none declared | Not authorized. P2-S03 is registered against issue #240, the only authorization source named for it, and #240 disclaims implementation authority in three places: "This issue is a durable requirement/backlog item. It does not authorize immediate implementation."; "Scope boundary - This issue does not: authorize immediate full UI rewrite; authorize removal of reference/n8n or n8n-editor-ui"; and "Implementation must be split into dedicated milestone tasks with Manager Master Prompts." No such dedicated milestone task exists: a search of open issues for Layer 3 core-workflow surfaces (workflow list/dashboard, editor, canvas, node picker/catalog, node configuration, connections, execution panel/history, import/export) returns no authorizing issue. Separately, the slice title itself carries Layer 6 (legacy UI decommission), which the scope boundary of #240 forbids and which permanent invariant 1 (no big-bang rewrite) and invariant 2 (original workflow JSON compatibility remains mandatory) contradict: the original editor must stay the compatibility oracle until each surface has a verified LEGO replacement. Recorded as blocked rather than executed - inventing a Manager Master Prompt would be inventing authorization (DEC-0020 notAuthorized). |
...
- **Freshness:** generated by `npm run lego:ai` from register 2.3.0 (fingerprint `213e2f824f3f440d`); `npm run lego:ai:check` fails when this section, the `.ai` pack or the register disagree.
