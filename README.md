...
Checkpoint-weighted earned points / current-delivery points: 13200 / 14600. Denominator: programs P0–P11 only (146 active slices). Future programs are excluded. 5 slice(s) declare checkpoints. 127 implemented slice(s) have no checkpoint list and contribute 100 each. 14 slice(s) have no checkpoint model and contribute 0. A 0 from a missing model is not a measured fraction of that slice.
...
P6   realtime ██████████████████░░  91.4%  completion  88.6%  31/35  status complete
...
| P6 | Node Registry / Node Runtime | 91.4% | 88.6% | 31/35 | complete |
...
```text
 1. `P6-S02` planned; realtime 0.0%; completion contribution 0.0%
 2. `P2-S02` planned; realtime 0.0%; completion contribution 0.0%
 3. `P2-S03` planned; realtime 0.0%; completion contribution 0.0%
```
...
- 🔴 **P6-S01** — Live community / private / custom node installation path over the P6 admission pipeline. Status: BLOCKED. Realtime 100.0%. Checkpoint-weighted from declared slice.checkpoints. Incomplete checkpoints contribute 0. Current checkpoint: —. Latest checkpoint: CP-04 Plan reproducibility, the purity and scope walls, and the contract-lock registration (completed, 24). Checkpoint evidence: CP-01: apps/n8n-lego/src/lego/install-source.mjs — 12 tests in lego-install-source.test.mjs: a community source registers with an attestation key and a remote locator, a private source registers with a pinned registry digest, a custom source registers with an operator approval and a local path, an optional policy object is carried through frozen, registration refuses a source that cannot be named (7 malformed ids), registration refuses an unknown kind (there is no fourth source), local is not remote (a custom source may not name a URI, a remote source may not name a path, and a bare word is not a path), every kind refuses the trust root another kind uses, a private source must pin a digest (6 malformed digests), a custom source must name who approved it and when (3 bad approvers, 5 bad timestamps), a community source must name the attestation key (4 malformed key ids), a source is registered against exactly one compiled epoch (6 malformed epoch numbers, 6 malformed epoch digests), registration is deterministic and the digest covers the trust root (changing the key, the trust root kind or the epoch number moves the source digest), and isInstallSource refuses a hand-made object that only looks like a source. Delivered in PR #326 (93f4bece), unmerged. [apps/n8n-lego/src/lego/install-source.mjs (registerInstallSource, isInstallSource, INSTALL_SOURCE_KINDS, INSTALL_SOURCE_TRUST_ROOT_BY_KIND, INSTALL_SOURCE_LOCATOR_KINDS, validateTrustRoot)]; CP-02: apps/n8n-lego/src/lego/install-source.mjs — 9 tests in lego-install-source.test.mjs: a community source is admitted only when the tenant policy is on (off by default, explicitly off, and a truthy non-true value all refused), private and custom sources need explicit allow-listing rather than the community switch (allow-listing the other kind is not allow-listing this one, and both may be allowed together), absence of a policy is a refusal for all three kinds, a source pinned to another epoch is refused with the anti-rollback reason named, no epoch supplied is unknown and unknown is never a pass, admission re-checks the trust root rather than trusting registration (an edited record is re-judged), admission refuses anything that is not a registered source (6 shapes), and admission is deterministic for one source and one policy with every check frozen. Delivered in PR #326 (93f4bece), unmerged. [apps/n8n-lego/src/lego/install-source.mjs (admitInstallSource, INSTALL_SOURCE_VERDICTS)]; CP-03: apps/n8n-lego/src/lego/install-source.mjs — 8 tests in lego-install-source.test.mjs: a fully admitted path is ready end to end and opens a transaction (every step ready, every step citing its contract, the format line exact), a refused source never reaches the node (step 1 refused with the source verdict as its detail, step 2 refused, opens-transaction=false), a refused node never opens a transaction and names the step that refused, a missing verdict is unknown rather than pass for both a missing admission plan and a missing closure with transact inheriting the unknown, an incomplete plan stays incomplete even when nothing was refused, every kind walks the same path and the plan names the kind, the read helpers refuse anything that is not a plan this contract produced, and a plan is frozen end to end and cannot be edited under a caller. Delivered in PR #326 (93f4bece), unmerged. [apps/n8n-lego/src/lego/install-source.mjs (planLiveInstall, isLiveInstallPlan, liveInstallOpensTransaction, formatLiveInstallPlan, INSTALL_SOURCE_PATH, INSTALL_SOURCE_CITATIONS)]; CP-04: apps/n8n-lego/src/lego/install-source.mjs + apps/n8n-lego/test/lego-install-source.test.mjs — 6 tests: the contract identity, operations and permissions are canonical, every kind names the trust root and locator kind it requires with every path step citing a contract, describeInstallSource is a census with no state and no arguments, the plan is reproducible (identical inputs give one digest; a refused node, another source and another epoch each move it), P6-S01 is pure (the only node import is the hash, node-registry is the ONE repo import, and importing admission-explain, dependency-closure, package-transaction, supply-chain, registry-compiler, registry-integrity, registry or freshness would be re-deciding), and the scope walls are written down in INSTALL_SOURCE_RULES. Registered as contract-lock row 101 (node.install-source@0.1.0, domain node-registry, 26 locked exports) with the node-registry domain path added. Delivered in PR #326 (93f4bece), unmerged. [apps/n8n-lego/src/lego/install-source.mjs (describeInstallSource, INSTALL_SOURCE_RULES, INSTALL_SOURCE_INPUT_SCHEMA_VERSION); apps/n8n-lego/src/lego/contracts/contract-lock.json row 101; apps/n8n-lego/src/lego/manifest/domains.json]. Blocker: the self-hosted runner fleet, not the work: all 10 self-hosted runners report status offline with 0 online, so PR #326's six required self-hosted jobs (Level 0 Check & Format, Level 1 Affected Tests, Level 2 Workspace Tests linux and windows, Level 2 Conformance LEGO & Node Catalog, Windows worker portability probe) are queued and have not run. A DEC-0015 retry was refused by the API (HTTP 403, the workflow run is still active) and with 0/10 runners online a retry would only re-queue, so the merge is blocked on WAITING_RUNNER. The contract itself is delivered and locally verified in PR #326 (93f4bece) and the three GitHub-hosted required jobs completed success.. Last progress update: 2026-09-26T00:00:00Z — BLOCKED on the self-hosted runner fleet, not on the work. The contract is delivered and verified locally in PR #326 (93f4bece): node.install-source@0.1.0 with registerInstallSource / isInstallSource / admitInstallSource / planLiveInstall / isLiveInstallPlan / describeInstallSource, registered as contract-lock row 101 with the node-registry domain path, and 39/39 tests in lego-install-source.test.mjs. The full backend suite is 2701/2704 (the 3 failures pre-existing on 0d11963d in rest.test.mjs), frontend-lego 451/451, workforce 7/7, the arch gate and its self-test, the foundation gate and its self-test, capability conformance and scale-out all OK, and lego:ai:check in sync at 101 files. PR #326's three GitHub-hosted jobs (Unit + integration tests and release package, Clean clone → start → health → browser smoke → restart, Backend LEGO architecture gate) all completed success. Its six required self-hosted jobs (Level 0 Check & Format, Level 1 Affected Tests, Level 2 Workspace Tests linux and windows, Level 2 Conformance LEGO & Node Catalog, Windows worker portability probe) are queued and have not run: all 10 self-hosted runners report status offline with 0 online, continuously, for 27 minutes of polling. A DEC-0015 retry was attempted and refused by the API (HTTP 403, the workflow run is still active so a run cannot be re-run), and with 0/10 runners online a retry would only re-queue. mergeable_state is unstable and mergeable is true, so the merge is blocked on WAITING_RUNNER rather than on anything the branch did. No merge was attempted while any required check was pending. PR #326 stays open and the branch is intact; the slice returns to the delivery path the moment the runner fleet reports online. The four checkpoints are completed with real evidence because the work is done and locally verified — earned progress is kept, and the slice is blocked rather than implemented, because a merge that has not been verified is not a delivery.. Completion contribution: 0%.

Planned queue (planned ≠ authorized): `P6-S02` → `P2-S02` → `P2-S03`.
...
- **Realtime Delivery Progress:** **91.4%** `██████████████████░░ 91.4%`
- **Slice Completion:** **88.6%** (31 / 35 implemented). Remaining 4. 31 implemented, 1 planned, 1 blocked, 2 proposed.
- **Program status:** complete. Program status is not a percentage and is not 100% just because the word is complete.
...
- **Checkpoint model:** 1 slice(s) declare checkpoints; 31 implemented slice(s) use the legacy 100 rule; 3 slice(s) have no checkpoint model and stay at 0.0%.
...
| `P6-S01` | Live community / private / custom node installation path over the P6 admission pipeline | Live community / private / custom node installation path over the P6 admission pipeline | 🔴 Blocked | 100.0% | 0.0% | — | CP-04 Plan reproducibility, the purity and scope walls, and the contract-lock registration (completed, 24) | the self-hosted runner fleet, not the work: all 10 self-hosted runners report status offline with 0 online, so PR #326's six required self-hosted jobs (Level 0 Check & Format, Level 1 Affected Tests, Level 2 Workspace Tests linux and windows, Level 2 Conformance LEGO & Node Catalog, Windows worker portability probe) are queued and have not run. A DEC-0015 retry was refused by the API (HTTP 403, the workflow run is still active) and with 0/10 runners online a retry would only re-queue, so the merge is blocked on WAITING_RUNNER. The contract itself is delivered and locally verified in PR #326 (93f4bece) and the three GitHub-hosted required jobs completed success. |
...
- **Freshness:** generated by `npm run lego:ai` from register 2.3.0 (fingerprint `9b4cab41642ec5f0`); `npm run lego:ai:check` fails when this section, the `.ai` pack or the register disagree.
