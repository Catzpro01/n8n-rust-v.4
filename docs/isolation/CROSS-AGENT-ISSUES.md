...
## ISSUE-023 — AUDIT OF ISSUE-015/017 EVIDENCE (2026-09-18, Workflow LEGO owner)

**Reported by:** Workflow LEGO owner (agent-1 lineage, Arena session `arena/01a0b1cb-n8n-rust-v-4`)
**Affected:** Agent 5 (ledger accuracy), Rust port owner, verification gate
**Type:** Missing golden fixture + one incorrect claim in an OPEN HIGH issue
**Severity:** MEDIUM (evidence gap; the underlying divergence class is real)

### 1. The fixture ISSUE-015 relies on did not exist

ISSUE-015 says: *"Agent 5 already shipped the fixture that would catch it:
`tests/reference/04-disabled-node/` … No crate consumes it — see ISSUE-012 / R1."*

```text
$ git ls-tree -r --name-only origin/main tests/reference/ | grep disabled
(no output — the tree holds 01-, 02-, 03-, README.md, agent-4, baseline,
 connection, execution-data, expression, harness, workflow-rust)
```

So R1 was under-stated for this case: there was nothing to consume. The fixture
now exists (`tests/reference/04-disabled-node/`), with `expected.json`
**recorded** from `n8n-workflow@2.9.1` using real `n8n-nodes-base@2.9.1` node
classes — 101 calls, regenerated by
`npm --prefix packages/workflow-recon run record:golden`, never hand-written.

### 2. Correction: parent/child traversals do NOT skip disabled nodes

ISSUE-015: *"for a workflow containing a disabled node, `get_parent_nodes` /
`get_child_nodes` in Rust will return a **different set** than n8n 2.9.4."*

Recorded from the oracle (`tests/reference/04-disabled-node/expected.json`):
...
Both sets include the disabled nodes. `common/get-connected-nodes.ts` — the single
implementation behind both (`get-parent-nodes.ts`, `get-child-nodes.ts`) — contains
no `disabled` branch. The divergence class is real but narrower: `getHighestNode`
(`workflow.ts:498,553`) and start-node selection (`:824,839,853`). A port that
filtered disabled nodes out of parent/child sets would introduce a divergence.

ISSUE-017's claim (a disabled trigger must not become the start node) **holds**;
it is now pinned by test with the recorded answer
`getStartNode() == "Schedule Trigger"`.

One asymmetry worth pinning for whoever ports this: `:498` tests
`disabled === false` while `:553` tests `disabled !== true`, so for a node with no
incoming main connection `getHighestNode` returns `["X"]` when `disabled: false`
and `[]` when the key is absent. Observed on both sides.

### 3. Delivered (TypeScript, per PROJECT_RULES rule 1 — no Rust touched)

`packages/workflow-recon/` reconstructs the Workflow graph surface with every
method annotated to its reference line range, plus a differential test against the
live `n8n-workflow@2.9.1` (4 golden fixtures + 6 synthetic cases) and an offline
replay of the recorded golden.

```text
$ npm --prefix packages/workflow-recon test            # 18 pass / 0 fail / 0 skipped
$ npm --prefix packages/workflow-recon run test:golden # 7 pass, with .runtime removed
$ npm --prefix packages/workflow-recon run typecheck   # tsc --noEmit, exit 0
```

Record: `docs/isolation/workflow-disabled-fidelity.md`.

### 4. Gate status (unchanged, and not this task's to fix)

`bash tests/integration/run_gate.sh --offline-only` is still **BLOCKED**: Stage 1
`25/26` (the one FAIL is the Phase-2 Rust guard, 22 artifacts under `crates/`),
Stage 2 `FAIL` for the same reason. Before this change Stage 1 was `20/21` with the
identical single FAIL, so nothing regressed.

The contradiction is real and needs an orchestrator decision, not a unilateral
edit: `PROJECT_RULES.md` rule 1 forbids Rust in `crates/`/`apps/`, while the
Phase-3 Rust workspace and `tools/rust-offline-rig/` exist and are actively
verified (ISSUE-022 records 88 passing cargo tests). Either the guard is scoped to
Phase 2 and must be gated on the current phase, or `crates/` must be emptied —
both are policy calls.

**Status:** OPEN (decision requested) — fixture and evidence half: CLOSED.
