...
| F6 | MEDIUM | §2 input is `WorkflowContract` (`nodes: NodeContract[]`, `connections`, …); determinism requirement: DFS visits nodes in `nodes[]` order, edges in output/index order, reports the **cycle path** | Takes `&[String]` names (loses node metadata such as `disabled`); adjacency built from `HashMap` iteration → **non-deterministic** edge order; reports only one node name, not the path | D7 expects `Cycle detected: A → B → A`; port yields `…involving node 'A'` (no path). **Revised 2026-09-17 (§5 / spec §13):** executable probe shows the witness is stable across hash seeds since the IndexMap switch (8ed00851) — F6 downgraded to *ordering follows JSON key order instead of §11.9*; the "depends on hash seed" claim above is withdrawn |
