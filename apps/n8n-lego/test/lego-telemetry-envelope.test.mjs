...
test('P9.1 publishes exactly one new contract row without transferring domain ownership', () => {
  ...
  assert.equal(lock.contracts.length, 95); // count-pins say 76 // count-pins say 75 // count-pins say 74 // count-pins say 73 // count-pins say 72 // count-pins say 71 // count-pins say 70 // count-pins say 69 // count-pins say 68 // count-pins say 67 // count-pins say 66 // count-pins say 65 // count-pins say 64 // count-pins say 63 // count-pins say 62 // count-pins say 61 // count-pins say 60 // count-pins say 59 // count-pins say 58 // count-pins say 57 // count-pins say 56 // count-pins say 54 // count-pins say 53 // count-pins say 52 // count-pins say 51 // count-pins say 49 // count-pins say 48 // count-pins say 47 // count-pins say 46 // count-pins say 45 // count-pins say 44 // P9.1=40 + P3 Slice K execution.optimizer=41 + P6.1 node.registry=43 // P9 integration (Agent 1): P9.5-P9.22 add 18 observability.* rows on top of protected main 76 -> 94 (union, zero id collisions); P9 was developed on 24032a0c (57 rows). P5.1 adds the ninety-fifth (auth.principal).
