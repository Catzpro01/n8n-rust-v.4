...
test('P9.1 publishes exactly one new contract row without transferring domain ownership', () => {
  ...
  assert.equal(lock.contracts.length, 61); // P9.7 adds observability.telemetry-redaction@1.0.0; P9.8 adds observability.telemetry-sampling@1.0.0; count-pins say 61 // P9.6 adds observability.telemetry-buffer@1.0.0 // count-pins say 61 // P9.5 adds observability.semantic-event@1.0.0 // count-pins say 61 // count-pins say 61 // count-pins say 61 // count-pins say 61 // count-pins say 61 // count-pins say 61 // count-pins say 61 // count-pins say 61 // count-pins say 61 // count-pins say 61 // count-pins say 61 // count-pins say 61 // P9.1=40 + P3 Slice K execution.optimizer=41 + P6.1 node.registry=43
