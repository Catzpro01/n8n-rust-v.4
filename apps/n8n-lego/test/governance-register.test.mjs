...
test('completion KPI is implemented/total and never counts verifying or blocked', () => {
  ...
  assert.equal(tally.percent, 88.2);
  assert.equal(tally.implemented, 134);
  ...
  assert.equal(metrics.current.implemented, 133);
  assert.equal(metrics.current.sliceCompletion, percent1(133, 146));
