...
test('the lock row is the fortieth: execution.optimizer@1.0.0, owner agent-1, exports byte-parity', () => {
  const row = ROWS.find((r) => r.id === 'execution.optimizer');
  assert.ok(row, 'execution.optimizer is locked');
  assert.equal(ROWS.length, 40, 'P3 Slice M added the thirty-ninth (execution.guard); P3 Slice K adds the fortieth (execution.optimizer) — count-pins say 40');
