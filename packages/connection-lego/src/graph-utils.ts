...
/**
 * 1:1 reconstruction of `reference/n8n/packages/workflow/src/graph/graph-utils.ts`
 * (n8n 2.9.4, commit `b6dc2787c45677a29a9612cd27eb911302961a83`, 273 lines).
 *
 * Owned by the Connection LEGO per `contracts/connection.contract.md` §7. Pinned by the
 * `getRootNodes` / `getLeafNodes` / `getInputEdges` / `getOutputEdges` / `hasPath` /
 * `parseExtractable` probes in `tests/reference/connection/02-multi-output` and `04-cycle`.
 *
 * Reference quirks reproduced on purpose:
 *
 * - **`buildAdjacencyList` keys a `Set` by object identity.** Two structurally identical
 *   connections coming from different slots are *different* `IConnection` objects and both are
 *   kept; a re-created object with equal contents is *also* kept. The adjacency list is
 *   therefore multiset-like. Pinned by `04-cycle/adjacency`.
 * - **`getRootNodes` / `getLeafNodes` / `hasPath` filter on `type === 'main'`** — `ai_*` edges
 *   are invisible to root/leaf/path analysis (`graph-utils.ts:117`, `:141`, `:159`).
 * - **`hasPath` is DFS with a `pop()` stack**, so it explores in reverse insertion order; only
 *   the boolean result is observable.
 * - **`parseExtractableSubgraphSelection` tolerates two degenerate selections**: no root but
 *   exactly one input node (`graph-utils.ts:225`), and no leaf but exactly one output node
 *   (`graph-utils.ts:246`). Both are reachable and both are asserted by the golden cases.
 */
