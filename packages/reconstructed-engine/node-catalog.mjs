/**
 * Built-in node catalog — native multi-locale registration (Agent 2).
 *
 * This is the Node Model LEGO's contribution to the Native Multi-Locale wave:
 * the human-facing metadata (labels, descriptions, parameter display names)
 * of the core built-in node types, expressed natively in the six official
 * locales (`id`, `jv`, `ar`, `zh`, `ru`, `en`) and registered through the
 * `registerTranslations(locale, catalog)` seam provided by Agent 1
 * (`localization.mjs` / `backend-localization-service.ts`).
 *
 * Ground truth for the English source text is the pinned n8n 2.9.4 reference
 * runtime (`n8n-nodes-base/dist/types/nodes.json`). The reference tree itself
 * is never modified.
 *
 * Policy (mirrors the Universal Locale Enforcer):
 *   1. only known human-facing fields are translated;
 *   2. the seven protected machine tokens (`name`, `type`, `value`, `inputs`,
 *      `outputs`, `routing`, `requestRules`) and `parameters` subtrees are
 *      never touched;
 *   3. unknown text is returned unchanged — nothing is guessed.
 *
 * Non-English catalogs additionally register the English source text as a
 * value alias (source text -> native text) so the enforcer's text lookup can
 * localize payloads that still carry English source values.
 */

import { SUPPORTED_LOCALE_CODES, normalizeSupportedLocale } from './localization.mjs';

export const BUILTIN_NODE_ALIASES = Object.freeze([
  'manualTrigger',
  'webhook',
  'scheduleTrigger',
  'cron',
...
export function nodeAliasOf(type) {
...
/* ------------------------------------------------------------------ */
/* English source catalog (n8n 2.9.4 nodes.json, byte-verified).        */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/* Native catalogs (one per non-English locale, identical key set).     */
/* ------------------------------------------------------------------ */

const ID_CATALOG = Object.freeze({
...
const JV_CATALOG = Object.freeze({
  'node.manualTrigger.label': 'Pemicu Manual',
...
const AR_CATALOG = Object.freeze({
...
const RU_CATALOG = Object.freeze({
...
const RAW_LOCALE_CATALOGS = Object.freeze({
  id: ID_CATALOG,
  jv: JV_CATALOG,
  ar: AR_CATALOG,
  zh: ZH_CATALOG,
 ...
 */
function withValueAliases(en, localized) {
  const out = { ...localized };
  for (const [key, nativeText] of Object.entries(localized)) {
    const enText = en[key];
    if (enText && enText !== nativeText && !Object.hasOwn(out, enText)) {
...
export const BUILTIN_NODE_CATALOG = Object.freeze({
  ...
  en: Object.freeze(EN_CATALOG),
});

/** Canonical (non-alias) translation keys of the built-in node catalog. */
export const BUILTIN_NODE_CATALOG_KEYS = Object.freeze(Object.keys(EN_CATALOG).sort());

function isKnownAlias(alias) {
  return BUILTIN_NODE_ALIASES.includes(alias);
}

/** Read a catalog entry: `node.<alias>.label | .description | .parameters.<name>`. */
export function getNodeCatalogEntry(alias, locale = 'id') {
  const target = normalizeSupportedLocale(locale);
  ...
  const label = table[`node.${alias}.label`] ?? enTable[`node.${alias}.label`];
  const description = table[`node.${alias}.description`] ?? enTable[`node.${alias}.description`];
  return { alias, label, description, locale: target };
}

/**
 * Register the built-in catalog on any service exposing
 * `registerTranslations(locale, translations)` — the UniversalLocaleEnforcer,
 * BackendLocalizationService, or the WorkflowExecutionEngine.
 */
export function registerBuiltInNodeCatalog(service) {
 ...
 */
export function localizeNodeMetadata(node, locale = 'id', service = null) {
  ...
  const translate = (text) => {
    ...
    const table = BUILTIN_NODE_CATALOG[target];
    return Object.hasOwn(table, text) ? table[text] : text;
  };
  ...
  if (alias && isKnownAlias(alias)) {
    const entry = getNodeCatalogEntry(alias, target);
    if (!entry) return out;
 ...
 */
export function createCommunityNodeCatalog(locale, packageName, nodes) {
 ...
 */
export function registerCommunityNodeCatalog(service, locale, packageName, nodes) {
  if (!service || typeof service.registerTranslations !== 'function') {
