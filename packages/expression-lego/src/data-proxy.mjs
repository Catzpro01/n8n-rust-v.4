/**
 * Expression LEGO — WorkflowDataProxy.
 * 1:1 behavioral port of n8n-workflow 2.9.4 src/workflow-data-proxy.ts.
 * Contract: contracts/expression.contract.md §4/§5/§6.
 * Golden reference: tests/reference/expression/* (observed n8n 2.9.4 runtime).
 */
...
export class WorkflowDataProxy {
	constructor(
		...
		this.runExecutionData = isScriptingNode(contextNodeName, workflow)
			? runExecutionData !== null
				? augmentObject(runExecutionData)
				: null
			: runExecutionData;
		this.connectionInputData = isScriptingNode(contextNodeName, workflow)
	...
	returnExecutionData(data, fullItem = false) {
		...
		if (Array.isArray(data)) return data.map((i) => i.json);
		return data.json;
	}

	/** workflow-data-proxy.ts nodeContextGetter */
			...
			{},
			{
				...
				get(target, name, receiver) {
					...
					if (!node) {
						throw new ExpressionError(
							`The workflow hasn't been executed yet, so you can't reference any context data`,
							{ runIndex: that.runIndex, itemIndex: that.itemIndex },
						);
					}

					if (
						name.toString() !== 'executionCount' &&
						!that.runExecutionData.executionData.hasOwnProperty(nodeName)
					) {
						return undefined;
					}

					if (name === 'executionCount') {
						const executionData = that.runExecutionData.executionData[nodeName];
						if (executionData === undefined) return 0;
						return executionData.length;
					}
					...
					if (name === 'runIndex') return executionData.length - 1;

					const taskData = executionData[name];
					...
					return new Proxy(
						...
						{
							has: () => true,
							ownKeys: (t) => Reflect.ownKeys(taskData),
							getOwnPropertyDescriptor: () => ({ enumerable: true, configurable: true }),
							get(t, propertyName) {
								if (propertyName === 'isProxy') return true;
								if (propertyName === 'runIndex') return executionData.length - 1;
								if (taskData[propertyName] === undefined) {
									throw new ExpressionError('No data found for item-index', {
										runIndex: that.runIndex,
										itemIndex: that.itemIndex,
									});
								}
								if (that.workflow.settings?.binaryMode !== BINARY_MODE_COMBINED) {
									return taskData[propertyName];
								}
								if (Array.isArray(taskData[propertyName])) {
									return taskData[propertyName].map((i) => i.json);
								}
								return taskData[propertyName].json;
							},
						},
					);
				},
	...
	/** workflow-data-proxy.ts getNodeExecutionData */
	...
	/** workflow-data-proxy.ts nodeDataGetter */
			...
			{ binary: undefined, data: undefined, json: undefined },
			{
				...
				get(target, name, receiver) {
					...
					if (['binary', 'data', 'json'].includes(name)) {
						const executionData = that.getNodeExecutionOrPinnedData({ nodeName, shortSyntax });

						if (executionData.length === 0 && !throwOnMissingExecutionData) return undefined;
	...
	prevNodeGetter() {
		...
		return new Proxy(
			{},
			{
				...
				get(target, name, receiver) {
					if (name === 'isProxy') return true;
					if (!that.executeData?.source) return undefined;
	...
	selfGetter() {
		...
		return new Proxy(
			{},
			{
				has: () => true,
				ownKeys() {
					return Reflect.ownKeys(that.selfData ?? {});
				},
				getOwnPropertyDescriptor() {
					return { enumerable: true, configurable: true };
				},
				get(target, name) {
					if (name === 'isProxy') return true;
					return (that.selfData ?? {})[name.toString()];
				},
			},
		);
	}

	/** workflow-data-proxy.ts nodeParameterGetter */
		...
		return new Proxy(node?.parameters ?? {}, {
			...
			get(target, name) {
				...
				if (name[0] === '&') {
					...
					if (!node || !Object.hasOwn(node.parameters ?? {}, name)) {
						return undefined;
					}
	...
	/** The main entry — workflow-data-proxy.ts getDataProxy */
	getDataProxy(opts) {
		const that = this;
		...
		const getPairedItem = (
			...
			incomingSourceData,
			initialPairedItem,
			usedMethodName = PAIRED_ITEM_METHOD.$GET_PAIRED_ITEM,
			nodeBeforeLast,
		) => {
			...
			const [firstMatch, ...rest] = matchedItems;
			if (rest.some((r) => r !== firstMatch)) {
		...
		const handleFromAi = (name, _description, _type = 'string', defaultValue) => {
			...
			if (!resultData) {
				...
				placeholdersDataInputData = inputData?.['ai_tool']?.[0]?.[itemIndex].json;
			}
		...
		const handleTool = (throwOnError = true) => {
			...
			try {
				...
				if (isNoExecutionDataError) return fallbackValue;
						...
						get(target, property, receiver) {
							...
							if (property === 'first') {
								...
								return (branchIndex, runIndex) => {
									...
									if (executionData[0]) return that.returnExecutionData(executionData[0], resolveFullItem);
									return undefined;
								};
							}
							if (property === 'last') {
					...
					get(target, property, receiver) {
						...
						if (property === 'first') {
							return (...args) => {
								...
								if (result[0]) return that.returnExecutionData(result[0]);
								return undefined;
							};
						}
						if (property === 'last') {
						...
						if (property === 'all') {
							return () => {
								const result = that.connectionInputData;
								if (result.length) return that.returnExecutionData(result);
								return [];
							};
						}
		...
		return new Proxy(base, {
			...
			get(target, name, receiver) {
				...
				if (name === '$binary') {
					return that.nodeDataGetter(that.contextNodeName, true, throwOnMissingExecutionData)?.binary;
				}
				if (name === '$tool') {
	...
	/** workflow-data-proxy.ts agentInfo (subset port — tool inventory for agent nodes). */
	agentInfo() {
		...
		return { memoryConnectedToAgent: false, tools: [] };
	}
}
