...
test('getWaitingExecutions returns waiting executions within time horizon', () => {
  const repository = new ExecutionRepository();
  const now = new Date('2026-09-18T10:00:00Z');
  const soon = new Date('2026-09-18T10:00:30Z');
  const later = new Date('2026-09-18T10:05:00Z');
  const id1 = repository.create({ mode: 'trigger', status: 'waiting', waitTill: soon, workflowData: {} });
  const id2 = repository.create({ mode: 'trigger', status: 'waiting', waitTill: later, workflowData: {} });
  const id3 = repository.create({ mode: 'trigger', status: 'running', waitTill: soon, workflowData: {} });

  const waiting = repository.getWaitingExecutions(60000, now);
  assert.equal(waiting.length, 1);
  assert.equal(waiting[0].id, id1);
  assert.deepEqual(waiting[0].waitTill, soon);
});
