...
import { NodeOperationError } from '../src/index.mjs';
...
test('NodeOperationError: messageMapping wins before the COMMON_ERRORS scan (node.error.ts L142-152)', () => {
  const e = new NodeOperationError(NODE, 'ETIMEDOUT-ish', { messageMapping: { etimedout: 'custom text' } });
  assert.equal(e.message, 'custom text');
  assert.deepEqual(e.messages, ['ETIMEDOUT-ish']);
});
...
test('NodeOperationError: string errors default to warning level even without options (L19-21 + L33)', () => {
  // no options object at all
  assert.equal(new NodeOperationError(NODE, 'plain').level, 'warning');
});
