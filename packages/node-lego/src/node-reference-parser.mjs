/**
 * Node-reference parser — turns `$('NodeName').item.json.x` style references inside node
 * parameters into sub-workflow input variables.
 *
 * Reconstruction target (n8n 2.9.4):
 *   reference/n8n/packages/workflow/src/node-reference-parser-utils.ts (whole file, 643 ln)
 *     - LazyRegExp                     L8-20
 *     - constants                      L35-54
 *     - hasDotNotationBannedChar       L56-60
 *     - backslashEscape                L62-66
 *     - dollarEscape                   L68-70
 *     - ACCESS_PATTERNS                L78-102
 *     - applyAccessPatterns            L104-126
 *     - convertToUniqueJsDotName       L128-145
 *     - convertDataAccessorName        L147-161
 *     - parseExpressionMapping         L163-262
 *     - extractExpressionCandidate     L264-283
 *     - parseCandidateMatch            L285-311
 *     - parse$jsonMatch                L313-322
 *     - parseReferencingExpressions    L324-352
 *     - applyParameterMapping          L354-381
 *     - resolveDuplicates              L383-424
 *     - applyExtractMappingToNode      L426-464
 *     - applyCanonicalMapping          L466-489
 *     - extractReferencesInNodeExpressions L491-643
 *
 * Deltas (contract §12.2):
 *   DELTA-01 — the reference imports four lodash helpers (`cloneDeep`, `escapeRegExp`, `isEqual`,
 *     `mapValues`). All four are reimplemented in `./lodash-lite.mjs`; note that lodash's
 *     `cloneDeep` (Date/RegExp/Map/Set-preserving) is a different function from the reference's
 *     own `utils.deepCopy` that `connection-io.mjs` uses, and each is used where upstream uses it.
 *   DELTA-02 — `OperationalError` is reconstructed locally in `./errors.mjs` (`name` stays
 *     `'Error'`, default level `'warning'`), like the rest of the boundary-local error model.
 *
 * Boundaries: leaf module — imports only `./errors.mjs` and `./lodash-lite.mjs`. The published
 * build exposes all five functions (`N25` compares them against the 2.9.1 build).
 */

import { OperationalError } from './errors.mjs';
import { cloneDeep, escapeRegExp, isEqual, mapValues } from './lodash-lite.mjs';
