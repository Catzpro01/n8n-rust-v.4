/**
 * jsonrepair — faithful dependency-free port of `jsonrepair@3.13.1` (ISC), the exact package
 * `n8n-workflow@2.9.1` imports in `utils.ts` L5 for `jsonParse(..., { repairJSON: true })`
 * (reference/n8n/packages/workflow/src/utils.ts L164-170).
 *
 * Ported verbatim from the published UMD bundle the reference build resolves
 * (`packages/workflow-lego/node_modules/jsonrepair/lib/umd/jsonrepair.js`, 903 ln): only the
 * wrapper was replaced with ESM named exports and the code was dedented — the scanner, the
 * character constants, the repair pipeline and the error positions are byte-for-byte upstream.
 * This closes DELTA-05's no-op `repairJSON` path: `jsonParse`'s default repair adapter is now
 * this module (an injected adapter still wins — see `type-validation.mjs`).
 *
 * Cite: https://github.com/josdejong/jsonrepair (v3.13.1, ISC).
 */
