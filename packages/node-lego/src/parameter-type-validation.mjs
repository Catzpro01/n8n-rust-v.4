/**
 * Parameter type validation — `validateNodeParameters` and the assertions behind it.
 *
 * Reconstruction target (n8n 2.9.4):
 *   reference/n8n/packages/workflow/src/node-parameters/parameter-type-validation.ts (272 ln)
 *     - assertUserInput / assertParamIsType / assertParamIsArray L13-89
 *     - resource-locator + object + primitive assertions L120-176
 *     - validateParameterType / validateParameterAgainstTypes L178-228
 *     - validateNodeParameters (public entry) L239-272
 *   Oracle: test/node-parameters/parameter-type-validation.test.ts (806 ln)
 *
 * The raised class is the reference's `NodeOperationError` (src/errors.mjs), which keeps
 * the reference's `name`, `level` and `node` identity — pinned by the differential.
 */

import assert from 'node:assert';

import { NodeOperationError } from './errors.mjs';
...
export function assertParamIsOfAnyTypes(parameterName, value, types, node) {
	...
	if (!isValid) assertUserInput(false, `Parameter "${parameterName}" must be ${types.join(' or ')}`, node);
}
...
function assertIsResourceLocator(parameterName, value, node) {
	assertUserInput(
		typeof value === 'object' && value !== null && '__rl' in value && 'mode' in value && 'value' in value,
...
function validateParameterAgainstTypes(parameterName, value, types, node) {
	...
	if (!isValid) {
		assertUserInput(false, `Parameter "${parameterName}" does not match any of the expected types: ${types.join(' or ')}`, node);
	}
}

/**
 * `validateNodeParameters(value, parameters, node)` — asserts the object shape and each
 * declared parameter; optional parameters are only validated when present.
 */
