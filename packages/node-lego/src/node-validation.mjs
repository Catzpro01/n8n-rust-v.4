/**
 * Node validation — credentials, connectivity and trigger-like detection.
 *
 * Reconstruction target (n8n 2.9.4):
 *   reference/n8n/packages/workflow/src/node-validation.ts (88 lines, whole file)
 *     - validateNodeCredentials L24-62  (required + displayOptions respected)
 *     - isNodeConnected       L67-86   (outgoing first, then incoming)
 *     - isTriggerLikeNode     L92-96
 *   Oracle: test/node-validation.test.ts (18 cases, all ported)
 */
...
export function validateNodeCredentials(node, nodeType) {
	...
	for (const credDesc of credentialDescriptions) {
		...
		if (!nodeCredential) {
			issues.push({ type: 'missing', displayName, credentialName });
			continue;
		}

		if (!nodeCredential.id) {
			issues.push({ type: 'not-configured', displayName, credentialName });
		}
	}
...
export function isNodeConnected(nodeName, connections, connectionsByDestination) {
	...
	if (connections[nodeName] && Object.keys(connections[nodeName]).length > 0) return true;

	// Check incoming connections
	if (connectionsByDestination[nodeName] && Object.keys(connectionsByDestination[nodeName]).length > 0) {
		return true;
	}
