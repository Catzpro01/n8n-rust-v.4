...
| `src/errors.mjs` | validation-boundary `NodeOperationError` + `OperationalError` + the `@n8n/errors` `ApplicationError`, plus the cancellation chain `ExecutionBaseError` -> `ExecutionCancelledError` -> `ManualExecutionCancelledError` (`sleepWithAbort` rejects with the last one; see ISSUE-024) |
...
| `src/lodash-lite.mjs` | DELTA-01: the `lodash/{get,isEqual,isObject,escapeRegExp,mapValues,cloneDeep,merge}` subset (`cloneDeep` ≠ the reference `deepCopy`: it keeps `Date`/`RegExp`/`Map`/`Set`/cycles; `merge` is the by-index array merge `utils.ts:323` relies on) |
...
```bash
npm test                 # 145 tests (node:test), no install step
...
The differential runs 29 scenario groups twice — against this package and against the
...
and error field shapes: **1822 agree / 0 diverge** (2 NOT-DIFFABLE surfaces:
`renameFormFields`, private `getPropertyValues`). A divergence is a bug in the port.
`cloneDeep`/`mapValues`/`escapeRegExp`/`merge` are not re-exported by the published build, so
those are compared against the reference build's own bundled `lodash` instead; `N28` adds
`updateDisplayOptions` plus the cancellation-error shape and `N29` drives the real
`sleep`/`sleepWithAbort` outcome paths on both sides.
...
Python-constant branch or its trailing-comma repair, `merge`'s by-index array rule,
`updateDisplayOptions`' merge, or the cancellation error class) produces one or more `DIVERGE`
lines — the harness is not vacuous. Removing an export (e.g. `sleep`) fails gate `N08` instead,
and a test that never settles fails gate `N03` (the cancelled-test guard).
