...
export const VOCABULARIES = Object.freeze([
  ...
  Object.freeze({
    ...
    about: 'state',
    values: Object.freeze(['created', 'ready', 'running', 'waiting', 'paused', 'completed', 'failed', 'cancelled']),
    provenance: Object.freeze({
      contract: Object.freeze({ id: 'ai.agent-machine', version: '1.1.0', owner: 'manager' }),
      kind: 'module',
      file: 'apps/n8n-lego/src/lego/agent-machine.mjs',
      symbol: 'AGENT_MACHINE_LIFECYCLE_STATES',
      read: 'values',
      note: 'the canonical agent vocabulary, quoted from the P2.16 foundation — the same states ai.agent-session@1.0.0 publishes, plus the machine-level ready state added by agentMachine.prepare at 1.1.0. waiting is bounded: entered only by an approval-required step and left only by cancel',
    }),
  }),
  Object.freeze({
    id: 'agentMachineOperation',
    question: 'Which Agent Machine operation is declared by ai.agent-machine@1.1.0?',
    about: 'operation',
    values: Object.freeze([
    ...
    provenance: Object.freeze({
      ...
      read: 'values',
      note: 'the nine canonical operations of 1.1.0: the seven 1.0.0 operations plus prepare and delegate (bounded bookkeeping). There is deliberately no close, send, stream, artifact or execute operation',
    }),
  }),
  Object.freeze({
    id: 'agentMachinePermission',
    question: 'Which permission is declared by ai.agent-machine@1.1.0?',
    about: 'permission',
    values: Object.freeze(['ai:agent:create', 'ai:agent:invoke', 'ai:agent:control', 'ai:agent:read', 'ai:agent:delegate']),
    provenance: Object.freeze({
      contract: Object.freeze({ id: 'ai.agent-machine', version: '1.1.0', owner: 'manager' }),
      kind: 'module',
      file: 'apps/n8n-lego/src/lego/agent-machine.mjs',
      ...
      read: 'values',
      note: 'the five canonical ai:agent:* words; 1.1.0 adds ai:agent:delegate for bounded delegation BOOKKEEPING (narrowed capability grants, clamped budget/deadline, never inheritance, never execution) and still publishes no execute-style permission — invoke is bounded bookkeeping control, not filesystem, shell, process or credential authority',
    }),
  }),
  Object.freeze({
...
export const DECLARED_OVERLAPS = Object.freeze([
  Object.freeze({
    ...
    values: Object.freeze({
      failed: 'a step can report failed work and a session can be in a failed state: one reported outcome, one lifecycle state, both quoted from their own contracts (ai.agent-machine@1.1.0 and ai.agent-session@1.0.0)',
