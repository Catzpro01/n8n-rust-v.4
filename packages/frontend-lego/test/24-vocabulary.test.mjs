...
test('every shared vocabulary is quoted with provenance, not re-invented', () => {
  ...
  for (const set of VOCABULARIES) {
    ...
    assert.ok(set.provenance.symbol.length > 0, `${set.id} names the declaration it read`);
...
test('a renamed-for-style vocabulary is declared rather than silently changed', () => {
  ...
  assert.deepEqual(unit.mirror, { declared: 'planned', available: 'implemented', partial: 'partial', unsupported: 'unsupported' });
  assert.equal(unit.extra.length, 2, 'both extra spellings carry a reason');
  for (const entry of unit.extra) assert.match(entry.reason, /canonical `(planned|implemented)`/);
});
