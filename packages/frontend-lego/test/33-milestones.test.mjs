...
test('the register is one machine-readable file at the canonical path, owned by the manager', () => {
  ...
  // overwritten here.
  assert.match(REGISTER.mainBaseline, /^7d964fd6/);
  assert.equal(REGISTER.currentMilestone, 'P2.22');
  assert.equal(REGISTER.previousCompletedMilestone, 'P2.21');
...
test('the baseline block protects main and names the branches of the current milestone', () => {
  ...
  assert.equal(REGISTER.mainBaseline, '7d964fd639257234472aed4029c160bcff86060e', 'the main baseline is the final protected-main commit');
