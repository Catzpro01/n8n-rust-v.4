/** Frontend contract tests for the bounded Agent Machine consumer (P2.16). */
...
const RECORD = Object.freeze({
  ...
  budgets: Object.freeze({ maxSteps: 4, maxDurationMs: 600000, maxContinuationBytes: 64, maxReferences: 8 }),
...
const ROW = Object.freeze({ contract: 'ai.agent-machine', version: '1.0.0' });
...
function declaration(overrides = {}) {
  return {
    ...
    permissions: ['ai:agent:create', 'ai:agent:invoke', 'ai:agent:control', 'ai:agent:read'],
    ...overrides,
  };
}
...
test('the Agent Machine catalog has canonical provenance and is internally aligned', () => {
  ...
  assert.equal(agentMachineCatalogAudit(catalog).ok, true);
  ...
  for (const affordance of ['startAffordance', 'stepAffordance', 'pauseAffordance', 'resumeAffordance', 'cancelAffordance', 'executeAffordance', 'approvalDecision']) {
...
test('the frontend quotes exactly the Agent Machine lifecycle, operations and permissions', () => {
  assert.deepEqual(vocabularyOf('agentMachineLifecycle').values, ['created', 'running', 'waiting', 'paused', 'completed', 'failed', 'cancelled']);
...
test('an exact handed-over record is rendered as bounded public data', () => {
  const view = describeAgentMachine({ record: RECORD, contractRows: [ROW] });
...
test('record validation preserves exact identity and rejects unknown lifecycle, outcome or budgets', () => {
  ...
  assert.throws(() => validateAgentMachineRecord({ ...RECORD, budgets: { ...RECORD.budgets, maxRetries: 99 } }), /bounded dimensions/);
...
test('alignment compares contract, exact operation and permission surfaces', () => {
  ...
  const fork = checkAgentMachineAlignment({ declaration: declaration({ operations: [...declaration().operations, 'agentMachine.delegate'] }) });
